 ...
 */
@WebServlet("/MasteritemtagList")
@MultipartConfig
public class MasteritemtagList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if ("MasteritemtagForm".equals(transition)) {
        	// フォワード
		    ...
		    synchronized (this) { // 同期化してリクエストを1回だけ処理する
		        if (session.getAttribute("importProcessed") == null) { // セッション内でインポートが未処理かどうかを確認
		            ...
		            if (filePart != null && filePart.getSize() > 0) { // ファイルがアップロードされているか確認
		                List<Masteritemtag> importedList = null;
		                try {
							importedList = dao.importOne(filePart);
						} catch (NamingException e1) {
							e1.printStackTrace();
						} // CSVファイルをインポートしてリストに変換
						session.setAttribute("importProcessed", true); // インポート処理済みフラグをセッションに設定
		                for (Masteritemtag masteritemtag : importedList) { // インポートされた各データをデータベースに保存
		                    try {
		                        dao.save(masteritemtag); // データベースに保存
		                    } catch (Exception e) {
